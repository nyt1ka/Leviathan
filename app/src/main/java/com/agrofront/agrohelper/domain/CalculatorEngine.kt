...
object CalculatorEngine {
    ...
    fun calculate(
        ...
        val totalMixes = if (mixTank > 0.0 && totalSolution > 0.0) ceil(totalSolution / mixTank).toInt() else 0
        ...
        val totalFlights = if (droneTank > 0.0 && totalSolution > 0.0) ceil(totalSolution / droneTank).toInt() else 0
        ...
        return WorkCalculation(
            ...
            productPerFullMix = productForSolutionVolume(if (fullMixes > 0) mixTank else 0.0, solutionRate, productRate),
            productForLastMix = productForSolutionVolume(lastMix, solutionRate, productRate),
            fullFlights = fullFlights,
            lastFlightLiters = lastFlight,
            totalFlights = totalFlights,
            productPerFullFlight = productForSolutionVolume(if (fullFlights > 0) droneTank else 0.0, solutionRate, productRate),
            productForLastFlight = productForSolutionVolume(lastFlight, solutionRate, productRate)
        )
    }
    ...
    fun productForSolutionVolume(
        ...
        if (solutionVolumeLiters <= 0.0 || solutionRateLHa <= 0.0 || productRatePerHa <= 0.0) return 0.0
        return (solutionVolumeLiters / solutionRateLHa) * productRatePerHa
    }
    ...
    fun parseBatteryDurationSeconds(input: String): Int {
        val value = input.trim()
        ...
        if (minutes < 0 || seconds !in 0..59) return 0
        return minutes * 60 + seconds
    }

    fun estimateTotalWorkSeconds(
        totalSolutionLiters: Double,
        droneTankLiters: Double,
        averageBatterySeconds: Int
    ): Int {
        if (totalSolutionLiters <= 0.0 || droneTankLiters <= 0.0 || averageBatterySeconds <= 0) return 0
        val equivalentTankCycles = totalSolutionLiters / droneTankLiters
        return (equivalentTankCycles * averageBatterySeconds).roundToInt()
    }

    fun formatLongDuration(seconds: Int): String {
        if (seconds <= 0) return "—"
    ...
    private fun normalizedRemainder(total: Double, capacity: Double): Double {
        ...
        return if (remainder < 0.000001 || capacity - remainder < 0.000001) 0.0 else remainder
    }
}
